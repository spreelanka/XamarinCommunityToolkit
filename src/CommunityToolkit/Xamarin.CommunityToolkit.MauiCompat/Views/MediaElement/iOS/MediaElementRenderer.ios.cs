...
{
	using Xamarin.CommunityToolkit.MauiCompat; public class MediaElementRenderer : ViewRenderer<ToolKitMediaElement, UIView>
	{
		...
		void PlayedToEnd(NSNotification notification)
		{
			...
			else
			{
				...
				{
					(Microsoft.Maui.Controls.Application.Current?.Handler.MauiContext?.Services.GetService(typeof(ILogger)) as ILogger)?.LogWarning("MediaElement", $"Failed to play media to end: {e}");
				}
			}
		}
		...
		protected virtual void Play()
		{
			...
			if (err != null)
				(Microsoft.Maui.Controls.Application.Current?.Handler.MauiContext?.Services.GetService(typeof(ILogger)) as ILogger)?.LogWarning("MediaElement", "Failed to set AVAudioSession Category {0}", err.Code);

			audioSession.SetMode(AVAudioSession.ModeMoviePlayback, out err);
			if (err != null)
				(Microsoft.Maui.Controls.Application.Current?.Handler.MauiContext?.Services.GetService(typeof(ILogger)) as ILogger)?.LogWarning("MediaElement", "Failed to set AVAudioSession Mode {0}", err.Code);

			err = audioSession.SetActive(true);
			if (err != null)
				(Microsoft.Maui.Controls.Application.Current?.Handler.MauiContext?.Services.GetService(typeof(ILogger)) as ILogger)?.LogWarning("MediaElement", "Failed to set AVAudioSession Active {0}", err.Code);
		...
		void MediaElementStateRequested(object? sender, StateRequested e)
		{
			switch (e.State)
			{
				...
				case MediaElementState.Stopped:
					...
					if (err != null)
						(Microsoft.Maui.Controls.Application.Current?.Handler.MauiContext?.Services.GetService(typeof(ILogger)) as ILogger)?.LogWarning("MediaElement", "Failed to set AVAudioSession Inactive {0}", err.Code);
